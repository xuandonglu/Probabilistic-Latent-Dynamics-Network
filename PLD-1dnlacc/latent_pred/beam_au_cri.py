import torch
import numpy as np
import torch.nn as nn
import torch.nn.functional as F
from utilities3 import *
from timeit import default_timer
import matplotlib.pyplot as plt
import time
import scipy.io as io
from sklearn.decomposition import PCA
from beam_au import SpectralConv2d_fast, FNO

ntest = 100
batch_size = 2
path_model = f'model/beam1d_au_N4900_p51_drop2'
model = torch.load(path_model)

################################################################
# load data
################################################################
with h5py.File('../reduction/data/data.hdf5', 'r') as f:
    u = f['acc_5000_501_1024'][:]
    force = f['f_5000_1024'][:]
u = torch.from_numpy(u).float()
S = u.shape[1]
T = u.shape[2]

fin = u[:, model.mea_loc.cpu()].permute(0, 2, 1)
x_normalizer = UnitGaussianNormalizer(fin[:1000].unsqueeze(1))
y_normalizer = UnitGaussianNormalizer(u[:1000])

test_x = fin[-ntest:]
test_y = u[-ntest:]
test_x = test_x.reshape(ntest, 1, T, test_x.shape[-1]).repeat([1, S, 1, 1])

test_x = x_normalizer.encode(test_x)
# y_normalizer.cuda()

test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(test_x, test_y), batch_size=batch_size)
device = torch.device('cuda')

MSELoss = nn.MSELoss(reduction='mean').cuda()
def evaluate_uncertainty(mu, var_total, y_true, z=1.96):
    std_total = torch.sqrt(var_total)

    mse = torch.mean((mu - y_true) ** 2)

    nll = 0.5 * torch.log(2 * torch.pi * var_total) + 0.5 * ((y_true - mu)**2) / var_total
    nll_mean = torch.mean(nll)

    sharpness = torch.mean(2 * z * std_total)

    lower = mu - z * std_total
    upper = mu + z * std_total
    covered = ((y_true >= lower) & (y_true <= upper)).float()
    coverage = torch.mean(covered)
    ece = torch.abs(coverage - 0.95)

    return {
        'MSE': mse.item(),
        'NLL': nll_mean.item(),
        'Sharpness': sharpness.item(),
        'ECE': ece.item()
    }

with torch.no_grad():
    test_loss = 0
    results  = {
    'MSE': 0.0,
    'NLL': 0.0,
    'Sharpness': 0.0,
    'ECE': 0.0}
    model.train()
    n = -1
    for x, y in test_loader:
        x = x.cuda()
        orig_mean = []
        orig_au_var = []
        t1 = default_timer()
        for _ in range(200):
            out = model(x.permute(0, 2, 1, 3)).permute(0, 2, 1, 3)
            orig_mean.append(out[..., 0].cpu())
        orig_mean = torch.stack(orig_mean, dim=0)
        orig_eu_var = orig_mean.var(dim=0) * (y_normalizer.eps + y_normalizer.std)
        orig_au_var = torch.exp(out[..., 1].cpu())
        orig_mean = y_normalizer.decode(orig_mean.mean(dim=0))
        orig_var = orig_au_var + orig_eu_var
        t2 = default_timer()
        loss = MSELoss(orig_mean, y)
        test_loss += loss.item() / ntest * batch_size
        batch_result = evaluate_uncertainty(orig_mean, orig_var, y, z=1.96)
        for k in results:
            results[k] += batch_result[k] / ntest * batch_size
        n += 1
        print(f"[eval] sample={n} | total_time={t2 - t1:.3f}s")
        io.savemat(f'pn.mat', {'rec': orig_mean[:2].detach().numpy(), 'test': y[:2].detach().numpy()})

        # for i in range(30):
        #     true = y[0, :, i * 13].detach().cpu()
        #     mean = orig_mean[0, :, i * 13].detach().cpu()
        #     std_eu = torch.sqrt(orig_eu_var[0, :, i * 13]).detach().cpu()
        #     std_au = torch.sqrt(orig_au_var[0, :, i * 13]).detach().cpu()
        #     N = 401
        #     plt.fill_between(torch.linspace(0, N-1, steps=N), mean-2*std_eu-2*std_au, mean+2*std_eu+2*std_au, color='green',
        #                      alpha=0.5, label='EU')
        #     plt.plot(true)
        #     plt.plot(mean)
        #     plt.title(i * 13)
        #     plt.show()

print(f"[eval] test_loss={test_loss:.4e}")
