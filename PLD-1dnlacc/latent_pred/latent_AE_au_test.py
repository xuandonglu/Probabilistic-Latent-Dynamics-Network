...
from latent_AE_au import SpectralConv2d_fast, FNO
# time.sleep(400)

ntest = 100
batch_size = 2
reduction_model = 'AE_d25'
path_model = f'model/beam1d_au_N4900_p51_drop2_{reduction_model}'
...
fin = u_latent[:, model.mea_loc.cpu()].permute(0, 2, 1)
x_normalizer = UnitGaussianNormalizer(fin[:1000].unsqueeze(1))
y_normalizer = UnitGaussianNormalizer(u_latent[:1000])

test_x = fin[-ntest:]
test_y = u[-ntest:]
test_x = test_x.reshape(ntest, 1, Z, test_x.shape[-1]).repeat([1, S, 1, 1])
...
with torch.no_grad():
    ...
    for x, y in test_loader:
        ...
        for _ in range(200):
            ...
            out_mean.append(out[..., 0])
            orig = AE.decode(y_normalizer.decode(out[..., 0]).reshape([batch_size * S, Z])).reshape([batch_size, S, T])
        ...
        out_au_var = torch.exp(out[..., 1])
        au_samples = []
        for _ in range(200):
            noise = torch.randn_like(out_mean)
            au_sample = out_mean + torch.sqrt(out_au_var) * noise
            au_samples.append(AE.decode(y_normalizer.decode(out_mean).reshape([batch_size * S, Z])).reshape([batch_size, S, T]).cpu())
        au_samples = torch.stack(au_samples, dim=0)
        orig_au_var = au_samples.var(dim=0)
        orig_var = orig_au_var + orig_eu_var
        t3 = default_timer()
