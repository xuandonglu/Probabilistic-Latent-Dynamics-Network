...
with torch.no_grad():
    ...
    for x, y in test_loader:
        ...
        for _ in range(1000):
            ...
            out_mean.append(out[..., 0].cpu())
        out_mean = torch.stack(out_mean, dim=0)
        out_eu_var = out_mean.var(dim=0) * (y_normalizer.eps + y_normalizer.std)
        out_au_var = torch.exp(out[..., 1].cpu())
        out_mean = y_normalizer.decode(out_mean.mean(dim=0))
        orig_mean = torch.einsum("bti,io->bto", out_mean, tr)
        orig_eu_var = torch.einsum("bti,io->bto", out_eu_var, tr ** 2)
        orig_au_var = torch.einsum("bti,io->bto", out_au_var, tr ** 2)
