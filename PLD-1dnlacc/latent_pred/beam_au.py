...
class FNO(nn.Module):
    def __init__(self, modes1, modes2, width):
        ...
        self.register_buffer('mea_loc', torch.tensor((20, 40, 460, 480)))
        self.fc0 = nn.Linear(2+len(self.mea_loc), self.width)
...
if __name__ == '__main__':
    ...
    epochs = 100
    learning_rate = 0.001
    model = FNO(modes, modes, width).cuda()
    model.register_buffer('train_grid', torch.arange(0, 501, step=10))
    path_model = 'model/beam1d_au_' + f'N{ntrain}_' + f'p{len(model.train_grid)}' + '_drop2'
    ...
    test_x = fin[-ntest:]
    test_y = u[-ntest:]
    train_x = train_x.reshape(ntrain, 1, T, train_x.shape[-1]).repeat([1, S, 1, 1])
