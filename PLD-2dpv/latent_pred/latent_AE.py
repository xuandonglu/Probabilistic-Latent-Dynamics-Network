...
if __name__ == '__main__':
    ...
    path_model = 'model/pv2d_' + f'N{ntrain}_' + f'p{len(model.train_grid)}' + f'_drop2_{reduction_model}'
    ...
    test_x = fin[-ntest:]
    test_y = u[-ntest:]
    train_x = train_x.reshape(ntrain, 1, Z, 1).repeat([1, T, 1, 1])
    ...
    for ep in range(epochs):
        ...
        for x, y in train_loader:
            x, y = x.cuda(), y.cuda()
            optimizer.zero_grad()
            out = model(x.permute(0, 2, 1, 3)).permute(0, 2, 1)[:, model.train_grid]
            out = AE.decode(out.reshape([batch_size * M, Z])).reshape([batch_size, M, X, Y])
            loss = MSELoss(out, y)
            loss.backward()
            optimizer.step()
        ...
        if (ep + 1) % 10 == 0:
            with torch.no_grad():
                ...
                for x, y in test_loader:
                    ...
                    out = AE.decode(out.reshape([batch_size * T, Z])).reshape([batch_size, T, X, Y])
                    loss = MSELoss(out, y)
